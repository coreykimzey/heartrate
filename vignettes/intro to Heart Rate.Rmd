...
ranged <- target_ranges(tester$`Resting Heart Rate`, time_data = tester$Date, age = 24)
...
This is just a plot of my resting heart rate. It looks like the plot might be cyclical. So Then i thought to compare across days of the week.
```{r}
plot(tester$Date, tester$"Resting Heart Rate", type = "l",
     ylab = "Resting Heart Rate(Beats Per Min)", xlab = "Date")
```



Interestingly, it seems that my resting heart rate is high on Monday and Friday. I'm not sure why that would be. This would probably something I would continually look at.
```{r}
...
```{r}
 TukeyHSD(aov(tester$`Resting Heart Rate` ~ tester$days_of_week))

```

#Answering Questions of HW 5
